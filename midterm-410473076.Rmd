...
```{r ans4}
data3$成績>=60 ->logiTrue
map(data3,~.[logiTrue]) ->passData
passData
```
...
```{r}
str_sub(twDates,3,5) -> yearCharactor
as.integer(yearCharactor) +1911 ->westYears
westYears
```
...
```{r}
sampledTranscriptData$科目名稱 =="科目204" ->logiTrue
sampledTranscriptData$學期成績[logiTrue] ->chosenGrades
...
```{r}
sampledTranscriptData$學屆 <-
  str_sub(sampledTranscriptData,2,4)
sampledTranscriptData$系碼 <-
  str_sub(sampledTranscriptData,5,6)
sampledTranscriptData
```
...
sampledTranscriptData$學系 <-
  str_replace(sampledTranscriptData$系級,"1","")
sampledTranscriptData$學系 <-
  str_replace(sampledTranscriptData$學系,"2","")
sampledTranscriptData$學系 <-
  str_replace(sampledTranscriptData$學系,"3","")
sampledTranscriptData$學系 <-
  str_replace(sampledTranscriptData$學系,"4","")
...
```{r}
sampledTranscriptData$學號=="410073409b" & sampledTranscriptData$學年==101 & sampledTranscriptData$學期==1 -> chosemLogi
sum(sampledTranscriptData$學期成績[chosenLogi]*sampledTranscriptData$學分數[chosenLogi])/sum(sampledTranscriptData$學分數[chosenLogi]) ->studentGPA
studentGPA
