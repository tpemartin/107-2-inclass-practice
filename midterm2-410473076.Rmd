...
homeworkData <- data.frame(
  ...
  作業1分數=c(10,9,5),
  作業2分數=c(10,10,9),
  stringsAsFactors=FALSE     #有這條姓名才會是character
)
homeworkData
```
...
email_content_fun <- function(input){
  ...
  str_replace(emailTextOut,"y1",as.character(input$作業1分數)) ->emailTextOut    #as.character
...
EconCourseSelectionInfo <- list(
  "101"=list(url="http://www.econ.ntpu.edu.tw/econ/files/Way/2016072195021.doc"),    #url=
...
search_courseRules <- function(entranceYear,EconCourseSelectionInfo){
  if(entranceYear %in% c(101,102,103)){
    EconCourseSelectionInfo[["101"]]$url -> output  #注意數字的引號""
  }else if(entranceYear==104){
    EconCourseSelectionInfo[["104"]]$url -> output
  }else if(entranceYear %in% c(105,106)){
    EconCourseSelectionInfo[["105"]]$url -> output
  }else if(entranceYear>=107){
    EconCourseSelectionInfo[["107"]]$url -> output
  }else{
    "很抱歉沒有您入學年的資訊，請打系辦電話" -> output
  }
  return(output)
}
search_courseRules
...
search_courseRules_flexible <- function(entranceYear,courseSelectionInfo){
  names(courseSelectionInfo) -> ruleYears   #因為各系頒布學年不同，所以要先找到該系頒布學年ruleYears
  as.numeric(ruleYears) -> numNumber
  numNumber <= entranceYear -> logiTrue     #numNumber <= entranceYear -> logiTrue
  if(any(logiTrue)){                        #any(logiTrue)
    max(numNumber[logiTrue]) -> maxNum      #numNumber[logiTrue]
...
names(EconCourseSelectionInfo2)[[3]] <- "106"                       #注意箭頭方向
EconCourseSelectionInfo2[["107"]]$`線上志願選課辦法` <-             #[["107"]]$`線上志願選課辦法`
...
invoiceList$UTC時間[nyLogical] <-
  with_tz(ymd_hms(invoiceList$`訂單時間(以訂單時區顯示)`[nyLogical],tz="America/New_York"),tzone="UTC")     #[nyLogical]

invoiceList$UTC時間[tpLogical] <-
  with_tz(ymd_hms(invoiceList$`訂單時間(以訂單時區顯示)`[tpLogical],tz="Asia/Taipei"),tzone="UTC")          #[tpLogical]
